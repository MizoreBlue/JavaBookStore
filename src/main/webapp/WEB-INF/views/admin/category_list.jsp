...
<body>
    <div class="layout">
        <div class="sidebar">
            ...
            <a href="${pageContext.request.contextPath}/backend/category" class="active">分类管理</a>
        ...
        <div class="main">
            ...
            <div class="card">
                <div style="margin-bottom:15px;">
                    <a href="${pageContext.request.contextPath}/backend/category?action=add" class="btn btn-success">+ 新增分类</a>
                </div>
                <table>
                    <tr><th>ID</th><th>分类名称</th><th>类型</th><th>排序</th><th>状态</th><th>操作</th></tr>
                    <c:if test="${empty pageResult.records}">
                        <tr><td colspan="6" style="text-align:center;color:#999;">暂无数据</td></tr>
                    </c:if>
                    <c:forEach items="${pageResult.records}" var="cat">
                        <tr>
                            <td>${cat.id}</td>
                            <td>${cat.name}</td>
                            <td>${cat.type}</td>
                            <td>${cat.sort}</td>
                            <td>${cat.status eq 1 ? '启用' : '禁用'}</td>
                            <td>
                                <a href="${pageContext.request.contextPath}/backend/category?action=edit&id=${cat.id}" class="btn btn-primary">编辑</a>
                                <a href="${pageContext.request.contextPath}/backend/category?action=delete&id=${cat.id}" class="btn btn-danger" onclick="return confirm('确认删除?')">删除</a>
                            </td>
                        </tr>
                    </c:forEach>
                </table>
                <div class="pagination">
                    <c:if test="${pageResult.pageNum > 1}">
                        <a href="${pageContext.request.contextPath}/backend/category?page=${pageResult.pageNum - 1}">上一页</a>
                    </c:if>
                    <span class="current">${pageResult.pageNum} / ${pageResult.pages}</span>
                    <c:if test="${pageResult.pageNum < pageResult.pages}">
                        <a href="${pageContext.request.contextPath}/backend/category?page=${pageResult.pageNum + 1}">下一页</a>
                    </c:if>
